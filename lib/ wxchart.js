...
function drawPointText(points, series, config, context) {
  ...
  points.forEach(function (item, index) {
    if (item !== null) {
      ...
      context.fillText(formatVal, item.x - measureText(formatVal) / 2, item.y + (series.fontOffset === undefined ? -8 : series.fontOffset));
    }
  });
  context.closePath();
  context.stroke();
...
var Charts = function Charts(opts) {
  ...
  var config$$1 = assign({}, config, opts.config);
